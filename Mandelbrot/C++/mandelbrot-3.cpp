...
// Define << operator so we can write pixel_t to stream
std::ostream& operator << (std::ostream &o, const pixel_t& pixel)
{
  o << pixel.R << pixel.G << pixel.B;
  return o;
}
...
int main(int argc, char **argv) {
  ...
  std::vector<pixel_t> pixels(pixels_x*pixels_y);
  ...
  std::ofstream output_file("mandelbrot.ppm", std::ios::out | std::ofstream::binary);
